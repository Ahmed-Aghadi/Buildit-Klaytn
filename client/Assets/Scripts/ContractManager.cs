...
public class ContractManager : MonoBehaviour
{
    ...
    public void updateEditedMap(Vector3Int position, CellType cell)
    {
        ...
        {
            map[position.x, position.z] = EMPTY;
        }
        else if (cell == CellType.Road)
        {
            map[position.x, position.z] = ROAD;
        }
        else if (cell == CellType.Structure)
        {
            map[position.x, position.z] = HOUSE;
        }
        else if (cell == CellType.SpecialStructure)
        {
            map[position.x, position.z] = SPECIAL;
        }
        else
        {
            map[position.x, position.z] = SPECIAL;
    ...
    public async void confirmMapUpdates()
    {
        int[] x;
        int[] y;
        int[] utilId;
